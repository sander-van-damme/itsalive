...
- DOM execution, document serialization/restoration, interaction capture, screenshots, logging hooks, canonical-root enforcement, durability checks, `window.application`/`window.agent` installation and similar origin-dependent logic should normally live in the shell-owned injectable runtime bundle.
...
- The root shell owns platform persistence, durable app documents, prompts, product policy, runtime API contracts, configuration, credentials, behavioral summaries, histories, logs, and other platform state.
...
- Treat `window.application` and `window.agent` as shell-owned APIs installed by the injected runtime. Keep policy, durable state, prompts, behavioral memory, and diagnostics shell-owned; local implementations exist only to bridge or perform browser-origin operations.
