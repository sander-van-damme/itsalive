...
function bootstrap(session: RuntimeSession) {
  ...
  expect(message).toEqual(expect.objectContaining({ appId: APP_ID, runtimeSource: RUNTIME_SOURCE }));
  expect(message).not.toHaveProperty("documentHtml");
...
describe("RuntimeSession", () => {
  ...
  it("uses the transferred MessagePort for runtime messages and execution", async () => {
    ...
    port.start();
    session.post({ type: "execute", code: "return 1" }, "req_execute");
  ...
  it("reloads by creating a fresh bootstrap without carrying saved document data", () => {
    const { session } = createSession();
    session.switchTo(APP_ID, ORIGIN, RUNTIME_SOURCE);
    ...
    expect(post).toHaveBeenCalledOnce();
    expect(post.mock.calls[0]![0]).toMatchObject({ type: "init", appId: APP_ID });
    expect(post.mock.calls[0]![0]).not.toHaveProperty("documentHtml");
  });
