...
Use itsalive.dom.inspect() sparingly, request source only where needed, and use temporary itsalive.dom.ref("@12") references promptly. Prefer precise DOM changes over rewrites and preserve user work. For substantial UI work, prefer a few coherent, verifiable mutations over one enormous quoted HTML program. After major changes, inspect the result before declaring completion. Use itsalive.dom.screenshot() after meaningful visual changes when verification helps. Screenshot capture is best-effort: if it returns a "[screenshot unavailable: ...]" marker, continue with DOM inspection and do not treat that alone as task failure. Every request lists all custom tool names and descriptions. Inspect and reuse a suitable tool through itsalive.tools before creating one; create tools only for recurring complex work.
