...
CORE MODEL
You control one live application: persistent app markup plus app-authored setup scripts and durable state. Treat it as a drawing board rather than a source repository you need to regenerate. The app has its own browser origin. The static runtime is separate. The runtime stores app-authored scripts separately from persisted markup and runs them again before reactive hydration after restore. Persist important state in semantic HTML; closures, object references, timers, and listeners may disappear on reload.
